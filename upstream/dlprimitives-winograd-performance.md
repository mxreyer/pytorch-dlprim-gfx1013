...
over 900 sweeps of the six layer shapes. All of it is in
`winograd_ksplit.patch` at https://github.com/mxreyer/pytorch-dlprim-gfx1013,
with the measurements in that repository's OPENCL-PERF.md (Findings 3, 10, 11,
12). I would like to know which of these you want as PRs and how you would
prefer them gated.

## 1. Backward kernels without `atomic_addf` (972 → 1,374 img/s)
...
reduce, so this wants to be selected when `__opencl_c_ext_fp32_global_atomic_add`
is absent and the device is not NVIDIA, with the atomic path kept.

## 2. Four access patterns (1,374 → 1,816 img/s, inference 4,433 → 6,418)

Each found by timing a wrong-but-cheap variant of one stage:

- `winograd_fwd.cl`: the transformed filters are read at a `C × 64`-byte lane
...
## 3. Register prefetch (1,816 → 2,055 img/s)
...
## 4. Optional fp16 inner loop (2,055 → 2,987 img/s)
